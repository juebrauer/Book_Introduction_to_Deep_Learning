...
# Turn on or off batch normalization here!
mlp_with_batch_normalization = True

# training data parameters
WINSIZE = 600
NR_CLUSTERS = 5
NR_SAMPLES_TO_GENERATE = 2000
...
The blog of Rui Shu describes quite good how to
implement batch normalization with TensorFlow:
http://ruishu.io/2016/12/27/batchnorm/
'''
def multilayer_perceptron(x, weights, biases, phase):

    # hidden layer #1 with RELU
    layer_1 = tf.add(tf.matmul(x, weights['h1']),
                     biases['b1'])
    if mlp_with_batch_normalization:
        layer1_normalized_acts =\
            tf.contrib.layers.batch_norm(layer_1,
            center=True,
            scale=True,
            is_training=phase,
            scope='layer1_normalized_acts')
        layer_1 = tf.nn.relu(layer1_normalized_acts)
    else:
    ...
    layer_2 = tf.add(tf.matmul(layer_1, weights['h2']),
                     biases['b2'])
    if mlp_with_batch_normalization:
        layer2_normalized_acts = \
            tf.contrib.layers.batch_norm(layer_2,
                                         center=True,
                                         scale=True,
                                         is_training=phase,
                                         scope='layer2_normalized_acts')
        layer_2 = tf.nn.relu(layer2_normalized_acts)
    else:
        layer_2 = tf.nn.relu(layer_2)

    # output layer with linear activation (no RELUs!)
    if mlp_with_batch_normalization:
        out_layer = tf.matmul(layer2_normalized_acts, weights['out']) \
                    + biases['out']
    else:
...
def visualize_decision_boundaries(
        the_session, epoch_nr, x_in, mlp_output_vec, phase):
    ...
    res = the_session.run(mlp_output_vec,
                          feed_dict={x_in: input_mat, phase: 0}
                          )
...
def build_TF_graph():
    ...
    mlp_output_vec = multilayer_perceptron(x_in, weights, biases, phase)
    ...
    '''
    Note: when training, the moving_mean and moving_variance need
    to be updated. By default the update ops are placed in
    tf.GraphKeys.UPDATE_OPS, so they need to be added as a
    ...
    return optimizer, mlp_output_vec, loss, x_in, y_out, phase



def MLP_training(data_samples, optimizer, mlp_output_vec,
                 loss, x_in, y_out, phase):
...
def main():
    ...
    optimizer, mlp_output_vec, loss, x_in, y_out, phase = build_TF_graph()
