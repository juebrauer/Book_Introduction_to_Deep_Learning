...
class som:
    ...
    """
    def train(self, input_vec, learn_rate, adapt_neighbors):

        self.nr_steps_trained += 1

        if (self.nr_steps_trained % 1000 == 0):
            print("SOM has been trained for",
                  self.nr_steps_trained, "steps.")
        ...
        # 3. now move the BMU a little bit into the direction
        #    of the input vector
        BMU_neuron = self.list_neurons[BMU_nr]
        BMU_neuron.adapt_to_vector(input_vec, learn_rate)


        # 4. now get the list of all neighbors of the BMU
        #    and move the neighbors a little bit into the
        #    direction of the input vector as well
        if (adapt_neighbors):

            neighbors = self.get_neighbors(BMU_nr)
